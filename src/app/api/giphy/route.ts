...
// Giphy API - using the public beta key
...
export async function GET(request: NextRequest) {
  ...
  const type = searchParams.get('type') || (query ? 'search' : 'trending');
  ...
  try {
    ...
    if (!response.ok) {
      return NextResponse.json({
        data: generateFallbackGifs(query || 'trending', parseInt(limit)),
...
function generateFallbackGifs(query: string, limit: number) {
  ...
  const items = categories[query.toLowerCase()] || categories.trending;
  return Array.from({ length: Math.min(limit, items.length * 3) }, (_, i) => ({
    id: `fallback-${query}-${i}`,
    type: 'gif',
